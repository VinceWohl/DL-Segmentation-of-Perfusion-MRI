...
class ComplementaryOnlyLoss(nn.Module):
    """
    Focused loss with ONLY complementary information enhancement.
    Uses BCE + Dice + Complementary loss (no spatial or boundary terms).
    """
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        # Complementary loss (ONLY complementary, no spatial or boundary)
...
class nnUNetTrainer_SharedDecoder_ComplementaryLoss(nnUNetTrainer):
    """
    Trainer with shared decoder and complementary loss ONLY (no spatial or boundary loss).
    Uses shared spatial features but independent binary predictions per hemisphere.
    """
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, 
        ...
        # Complementary loss weight ONLY - no spatial or boundary
        self.complementary_weight = 0.1  # Complementary information loss
        ...
        print("Shared Decoder with Complementary Loss ONLY")
    ...
    def _build_loss(self):
        """Build complementary loss function (complementary ONLY, no spatial or boundary)."""
        loss = ComplementaryOnlyLoss(
            bce_kwargs={},
            soft_dice_kwargs={
    ...
    def _save_validation_summary(self, validation_results, validation_folder):
        ...
        if validation_results:
            ...
            summary = {
                "experiment_info": {
                    "trainer": "nnUNetTrainer_SharedDecoder_ComplementaryLoss",
