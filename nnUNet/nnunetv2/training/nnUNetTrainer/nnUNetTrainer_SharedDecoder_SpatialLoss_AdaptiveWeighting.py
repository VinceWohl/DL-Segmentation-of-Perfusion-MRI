...
class AdaptiveSpatialSharedDecoderLoss(nn.Module):
    """
    SharedDecoder loss with adaptive spatial weight scheduling.
    Spatial weight increases progressively during training for better convergence.
    """
    def __init__(self, bce_kwargs, soft_dice_kwargs, weight_ce=1, weight_dice=1, 
                 spatial_weight_start=0.05, spatial_weight_end=0.15, dice_class=MemoryEfficientSoftDiceLoss):
        super().__init__()
        self.weight_dice = weight_dice
        self.weight_ce = weight_ce
        self.spatial_weight_start = spatial_weight_start
        self.spatial_weight_end = spatial_weight_end
        self.current_spatial_weight = spatial_weight_start
    ...
    def update_spatial_weight(self, current_epoch, max_epochs):
        """Update spatial weight based on training progress."""
        if max_epochs > 0:
            progress = min(current_epoch / max_epochs, 1.0)
            # Linear progression from start to end weight
            self.current_spatial_weight = (
                self.spatial_weight_start + 
                progress * (self.spatial_weight_end - self.spatial_weight_start)
            )
        else:
            self.current_spatial_weight = self.spatial_weight_start
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        # Enhanced consistency losses with adaptive weighting
        pred_probs = torch.sigmoid(net_output)
        spatial_loss = self._spatial_consistency_loss(pred_probs, target) if self.current_spatial_weight > 0 else 0
        
        total_loss = (self.weight_ce * bce_loss + 
                     self.weight_dice * dice_loss + 
                     self.current_spatial_weight * spatial_loss)
        
        return total_loss


class nnUNetTrainer_SharedDecoder_SpatialLoss_AdaptiveWeighting(nnUNetTrainer):
    """
    Trainer with shared decoder and adaptive spatial loss weight scheduling.
    Spatial weight increases progressively from 0.05 to 0.15 during training for better convergence.
    """
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, 
        ...
        # Set adaptive spatial loss weights  
        self.spatial_weight_start = 0.05
        self.spatial_weight_end = 0.15
        ...
        print("Shared Decoder with Adaptive Spatial Loss Weight Scheduling")
        print(f"Input channels ({num_input_channels}): {', '.join(channel_names)}")
        print(f"Spatial loss weight: {self.spatial_weight_start} → {self.spatial_weight_end} (adaptive)")
    ...
    def _build_loss(self):
        """Build adaptive spatial-enhanced loss function with BCE + Dice + Adaptive Spatial consistency."""
        loss = AdaptiveSpatialSharedDecoderLoss(
            bce_kwargs={},
            soft_dice_kwargs={
            ...
            weight_ce=1,
            weight_dice=1,
            spatial_weight_start=self.spatial_weight_start,
            spatial_weight_end=self.spatial_weight_end,
    ...
    def on_epoch_start(self):
        """Update spatial weight at the beginning of each epoch."""
        super().on_epoch_start()
        
        # Update spatial weight in loss function
        if hasattr(self.loss, 'update_spatial_weight'):
            self.loss.update_spatial_weight(self.current_epoch, self.num_epochs)
            current_weight = self.loss.current_spatial_weight
        elif hasattr(self.loss, 'loss') and hasattr(self.loss.loss, 'update_spatial_weight'):
            # Handle DeepSupervisionWrapper
            self.loss.loss.update_spatial_weight(self.current_epoch, self.num_epochs)
            current_weight = self.loss.loss.current_spatial_weight
        else:
            current_weight = self.spatial_weight_start
        
        if self.current_epoch % 100 == 0:  # Log every 100 epochs
            print(f"Epoch {self.current_epoch}: Spatial weight = {current_weight:.4f}")
    ...
    def _save_validation_summary(self, validation_results, validation_folder):
        ...
        if validation_results:
            ...
            # Get current spatial weight for reporting
            current_weight = self.spatial_weight_start
            if hasattr(self.loss, 'current_spatial_weight'):
                current_weight = self.loss.current_spatial_weight
            elif hasattr(self.loss, 'loss') and hasattr(self.loss.loss, 'current_spatial_weight'):
                current_weight = self.loss.loss.current_spatial_weight
            
            summary = {
                "experiment_info": {
                    "trainer": "nnUNetTrainer_SharedDecoder_SpatialLoss_AdaptiveWeighting",
                    "dataset": "Dataset001_PerfusionTerritories",
                    "configuration": "2d",
                    "fold": self.fold,
                    "num_epochs": self.num_epochs,
                    "loss_type": "bce_dice_spatial_adaptive",
                    "spatial_weight_start": self.spatial_weight_start,
                    "spatial_weight_end": self.spatial_weight_end,
                    "final_spatial_weight": float(current_weight),
