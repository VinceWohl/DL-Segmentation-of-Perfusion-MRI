...
class nnUNetTrainer_ML2Ch(nnUNetTrainer):
    ...
    # planner → 2 output channels
    def determine_num_output_channels(self, plans_manager, dataset_json) -> int:
        return 2

    def configure_loss(self):
        pos_weight = None  # optionally: torch.tensor([wL, wR], device=self.device)
        self.loss = BCEDiceLossMultiLabel(bce_weight=0.5, dice_weight=0.5,
    ...
    @torch.no_grad()
    def validate(self, do_mirroring: bool = True, use_gaussian: bool = True,
                 tiled: bool = True) -> Dict[str, Any]:
        """
        Validation:
          - forward with sigmoid
          - threshold at 0.5
          - export 2-channel predictions as NIfTI
          - compute per-channel Dice in [0,1]
        """
        self.network.eval()
        pred_thresh = 0.5
        ...
        for batch in self.val_data_loader:
            ...
            # export predictions case-wise
            for i in range(pred.shape[0]):
                props = batch['properties'][i] if 'properties' in batch else None
                export_multilabel_pred(pred[i].cpu().numpy(), props=props,
    ...
    def initialize(self, *args, **kwargs):
        ...
        # 1) disable the built-in multiclass pseudo-dice path
    ...
    def finish_online_evaluation(self):
        """
        2) hard-stop the base-class 'Pseudo dice [...]' printout.
        Some nnU-Net versions call this unconditionally at epoch end.
        We do nothing here and rely on our validate() instead.
        """
        return  # no-op
    ...
    def print_to_log_file(self, *args, also_print_to_console: bool = True, add_timestamp: bool = True):
        """
        Drop nnU-Net's built-in 'Pseudo dice [...]' line (multiclass softmax metric).
        Keep everything else (including our ML Dice print).
        """
        # Build a single text blob of what would be printed
        msg = " ".join(str(a) for a in args if a is not None)
        if "Pseudo dice" in msg:
            return  # swallow it
        return super().print_to_log_file(*args, also_print_to_console=also_print_to_console, add_timestamp=add_timestamp)