...
class ComplementarySeparateDecodersLoss(nn.Module):
    """
    SeparateDecoders loss with complementary information enhancement.
    Combines BCE + Dice + Complementary loss for balanced hemisphere learning.
    """
    ...
    def _complementary_loss(self, pred_probs: torch.Tensor, target: torch.Tensor):
        """
        IMPROVED Complementary information loss for perfusion territories.
        Enforces mutual exclusivity in ALL brain regions and proper anatomical constraints.
        """
        ...
        # 1. IMPROVED: Mutual exclusivity in ALL brain regions (not just background)
        # Create brain mask: any region with perfusion territory
        brain_mask = (target_left + target_right) > 0  # Any perfusion territory present
        overlap_penalty = torch.mean(pred_left * pred_right * brain_mask.float())
        
        # 2. IMPROVED: Encourage predictions in one hemisphere when other is inactive
        # Enforce anatomical constraint: if left territory present, right should be minimal
        left_only_regions = (target_left > 0) & (target_right == 0)
        right_only_regions = (target_right > 0) & (target_left == 0)
        
        exclusivity_loss = 0
        if left_only_regions.sum() > 0:
            # In left-only regions, penalize right hemisphere predictions
            exclusivity_loss += torch.mean(pred_right[left_only_regions])
        if right_only_regions.sum() > 0:
            # In right-only regions, penalize left hemisphere predictions  
            exclusivity_loss += torch.mean(pred_left[right_only_regions])
        
        # 3. Coverage consistency (maintain from original)
        ...
        return overlap_penalty + exclusivity_loss + coverage_loss
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        if self.weight_ce != 0:
            ...
            # Equal weighting - no bias toward either channel
...
class nnUNetTrainer_SeparateDecoders_ComplementaryLoss(nnUNetTrainer):
    """
    Trainer with separate decoders and complementary loss enhancement.
    Uses dedicated decoders for each hemisphere with complementary information constraints.
    """
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, 
        ...
        print("Separate Decoders with Complementary Loss Enhancement")
    ...
    def _build_loss(self):
        """Build complementary-enhanced loss function with BCE + Dice + Complementary."""
        loss = ComplementarySeparateDecodersLoss(
            bce_kwargs={},
            soft_dice_kwargs={
    ...
    def _save_validation_summary(self, validation_results, validation_folder):
        ...
        if validation_results:
            ...
            summary = {
                "experiment_info": {
                    "trainer": "nnUNetTrainer_SeparateDecoders_ComplementaryLoss",
                    "dataset": "Dataset001_PerfusionTerritories",
                    "configuration": "2d",
                    "fold": self.fold,
                    "num_epochs": self.num_epochs,
                    "loss_type": "bce_dice_complementary",
                    "complementary_weight": self.complementary_weight,
