...
class SeparateDecodersModule(nn.Module):
    """Custom module to replace the final conv layer with separate decoders."""
    ...
    def forward(self, x):
        left_out = self.left_decoder(x)
        right_out = self.right_decoder(x)
        return torch.cat([left_out, right_out], dim=1)


class DualDecoderNetwork(nn.Module):
    """
    Network with shared encoder but separate decoders for each hemisphere.
    This prevents one channel from dominating the learning process.
    """
    ...
    def _find_and_replace_final_conv(self, module, parent=None, name=None):
        ...
        for child_name, child in module.named_children():
            if hasattr(child, 'out_channels') and child.out_channels == 2:
                # Found the final conv layer - replace it
                new_module = SeparateDecodersModule(child)
    ...
    def forward(self, x):
        # Forward through the base network (now with separate decoders)
        return self.base_network(x)


class SpatialConsistencyLoss(nn.Module):
    """
    Enhanced loss with spatial consistency for perfusion territory segmentation.
    """
    def __init__(self, bce_kwargs, soft_dice_kwargs, weight_ce=1, weight_dice=1, 
                 spatial_weight=0.1, boundary_weight=0.05, complementary_weight=0.1, 
        ...
        # Edge detection kernel for boundary loss
    ...
    def _spatial_consistency_loss(self, pred_probs: torch.Tensor, target: torch.Tensor):
        """
        Compute spatial consistency losses for perfusion territories (vectorized).
    ...
    def _boundary_consistency_loss(self, pred_probs: torch.Tensor, target: torch.Tensor):
        """
        Encourage smooth boundaries and anatomically plausible transitions (vectorized).
        """
        # Move Sobel filters to same device and dtype as input
        sobel_x = self.sobel_x.to(pred_probs.device, pred_probs.dtype)
        sobel_y = self.sobel_y.to(pred_probs.device, pred_probs.dtype)
        
        # Process both channels at once: (B, 2, H, W) -> (B*2, 1, H, W)
        batch_size, num_channels = pred_probs.shape[:2]
        pred_reshaped = pred_probs.view(batch_size * num_channels, 1, pred_probs.shape[2], pred_probs.shape[3])
        target_reshaped = target.view(batch_size * num_channels, 1, target.shape[2], target.shape[3])
        
        # Compute gradients for all channels simultaneously
        ...
        # Boundary consistency: predicted boundaries should match target boundaries
        boundary_loss = F.mse_loss(pred_grad_mag, target_grad_mag)
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        if self.spatial_weight > 0:
            spatial_loss = self._spatial_consistency_loss(pred_probs, target)
        
        if self.boundary_weight > 0:
            boundary_loss = self._boundary_consistency_loss(pred_probs, target)
...
class nnUNetTrainer_SeparateDecoders_SpatialLoss(nnUNetTrainer):
    """
    Enhanced Separate Decoders trainer with spatial consistency loss for 
    perfusion territory segmentation.
    """
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, 
        ...
        # Enhanced loss weights - can be tuned
        self.spatial_weight = 0.1
        self.boundary_weight = 0.0   # Disabled to focus on complementary loss
        ...
        print("Enhanced Separate Decoders with Spatial + Complementary Loss")
        print(f"Input channels ({num_input_channels}): {', '.join(channel_names)}")
        print(f"Spatial loss weight: {self.spatial_weight}")
        print(f"Boundary loss weight: {self.boundary_weight}")
    ...
    def build_network_architecture(architecture_class_name: str,
        ...
        # Wrap it with dual decoder architecture
        network = DualDecoderNetwork(base_network)
        return network
        
    def _build_loss(self):
        """Build enhanced loss function with spatial consistency."""
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        if validation_results:
            ...
            print("\n" + "="*60)
            print("FINAL VALIDATION RESULTS - ENHANCED SPATIAL LOSS")
            print("="*60)
    ...
    def _save_validation_summary(self, validation_results, validation_folder):
        ...
        if validation_results:
            ...
            summary = {
                "experiment_info": {
                    "trainer": "nnUNetTrainer_SeparateDecoders_SpatialLoss",
