...
class SpatialSharedDecoderLoss(nn.Module):
    """
    SharedDecoder loss with spatial consistency enhancement only.
    Combines BCE + Dice + Spatial consistency loss.
    """
    def __init__(self, bce_kwargs, soft_dice_kwargs, weight_ce=1, weight_dice=1, 
                 spatial_weight=0.1, dice_class=MemoryEfficientSoftDiceLoss):
        super().__init__()
...
class nnUNetTrainer_SharedDecoder_SpatialLoss_Deterministic(nnUNetTrainer):
    """
    Trainer with shared decoder and multi-label head with spatial consistency enhancement.
    Uses shared spatial features with spatial loss to encourage smooth segmentation boundaries.
    
    DETERMINISTIC VERSION: Includes comprehensive seed control for reproducible results.
    This version addresses potential random initialization issues in the original trainer.
    """
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, 
        ...
        print("Shared Decoder with Spatial Loss Enhancement - DETERMINISTIC")
        ...
        print("✅ Deterministic training enabled for reproducibility")
        
    def _set_deterministic_training(self, seed: int = 12345):
        """
        Set deterministic training for reproducible results.
        This helps ensure consistent performance across runs.
        """
        ...
        print(f"🔒 Deterministic training configured with seed: {seed}")
    ...
    def _build_loss(self):
        """Build spatial-enhanced loss function with BCE + Dice + Spatial consistency."""
        loss = SpatialSharedDecoderLoss(
            bce_kwargs={},
            soft_dice_kwargs={
    ...
    def _initialize_weights_deterministically(self):
        ...
        This can help resolve training instability issues.
        """
        print("🎯 Applying deterministic weight initialization...")
        ...
        print("   ✅ Network weights initialized deterministically")
    ...
    def _save_validation_summary(self, validation_results, validation_folder):
        ...
        if validation_results:
            ...
            summary = {
                "experiment_info": {
                    "trainer": "nnUNetTrainer_SharedDecoder_SpatialLoss_Deterministic",
                    "dataset": "Dataset001_PerfusionTerritories",
                    "configuration": "2d",
                    "fold": self.fold,
                    "num_epochs": self.num_epochs,
                    "loss_type": "bce_dice_spatial",
                    "spatial_weight": self.spatial_weight,
