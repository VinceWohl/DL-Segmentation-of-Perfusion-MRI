...
class DualChannelBalancedLoss(nn.Module):
    """
    Balanced loss that ensures equal learning for both channels.
    """
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        # Compute Dice loss (handles both channels)
        dice_loss = self.dice(net_output, target) if self.weight_dice != 0 else 0
        
        # Compute BCE loss per channel with equal weighting
        if self.weight_ce != 0:
...
class nnUNetTrainer_SeparateDecoders(nnUNetTrainer):
    """
    ...
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, 
        ...
        print("Separate Decoders: Using dedicated decoders for each hemisphere")
    ...
    def _build_loss(self):
        """Build balanced loss function."""
        loss = DualChannelBalancedLoss(
            bce_kwargs={},
            soft_dice_kwargs={
    ...
    def _save_validation_summary(self, validation_results, validation_folder):
        ...
        if validation_results:
            ...
            summary = {
                "experiment_info": {
                    "trainer": "nnUNetTrainer_SeparateDecoders",
