...
class BaselineSharedDecoderLoss(nn.Module):
    """
    Baseline loss with only BCE and Dice loss for SharedDecoder baseline comparison.
    """
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor):
        ...
        total_loss = self.weight_ce * bce_loss + self.weight_dice * dice_loss
        
        return total_loss


class nnUNetTrainer_SharedDecoder_Deterministic(nnUNetTrainer):
    """
    Trainer with shared decoder and multi-label head for balanced hemisphere learning.
    Uses shared spatial features but independent binary predictions per hemisphere.
    
    DETERMINISTIC VERSION: Includes comprehensive seed control for reproducible results.
    This version ensures fair comparison by eliminating random initialization variability.
    """
    ...
    def _set_deterministic_training(self, seed: int = 12345):
        ...
        print(f"🔒 Deterministic baseline training configured with seed: {seed}")
        print("   - Python random seed: SET")
        print("   - NumPy random seed: SET")
        print("   - PyTorch random seed: SET") 
        print("   - CUDA random seed: SET")
    ...
    def _build_loss(self):
        """Build baseline loss function with only BCE and Dice loss."""
        loss = BaselineSharedDecoderLoss(
            bce_kwargs={},
            soft_dice_kwargs={
    ...
    def initialize_network(self):
        """Override to ensure our custom label manager is used consistently with deterministic weights."""
        # Re-enforce deterministic settings before network initialization
        self._set_deterministic_training()
        ...
        self._initialize_weights_deterministically()
        
    def _initialize_weights_deterministically(self):
        """
        Apply deterministic weight initialization to ensure reproducible results.
        This ensures fair comparison by eliminating random weight initialization variability.
        """
        print("🎯 Applying deterministic weight initialization for baseline...")
        
        def init_weights(m):
            if isinstance(m, nn.Conv2d):
                # Use Xavier/Glorot initialization with fixed seed
        ...
        # Apply initialization to the network
        self.network.apply(init_weights)
        print("   ✅ Baseline network weights initialized deterministically")
    ...
    def _save_validation_summary(self, validation_results, validation_folder):
        ...
        if validation_results:
            ...
            summary = {
                "experiment_info": {
                    "trainer": "nnUNetTrainer_SharedDecoder_Deterministic",
                    "dataset": "Dataset001_PerfusionTerritories",
                    "configuration": "2d",
                    "fold": self.fold,
                    "num_epochs": self.num_epochs,
                    "loss_type": "baseline_bce_dice",
