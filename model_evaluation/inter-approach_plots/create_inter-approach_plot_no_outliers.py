#!/usr/bin/env python3
"""
Inter-Approach Segmentation Comparison Box Plots for Cross-Validation Results (No Outliers)
...
class InterApproachPlotterNoOutliers:
    ...
    def create_input_config_plot(self, config_name, files_dict, save_path):
        ...
        for approach_name in approach_order:
            if approach_name in files_dict and files_dict[approach_name]:
                ...
                print(f"\nLoading {approach_name} data (removing outliers):")
        ...
        ax.set_title(f'{config_display} Input Configuration: DSC by Segmentation Approach and Hemisphere (No Outliers)\n'
        ...
        print(f"Saved {config_name} plot (no outliers): {save_path}")
    ...
    def generate_all_plots(self):
        ...
        print("Inter-Approach Segmentation Comparison Box Plot Generator (No Outliers)")
        print("=" * 70)
        ...
        for config_name, approach_files in files_dict.items():
            ...
            # Create save path
            save_path = self.plots_dir / f"inter_approach_comparison_{config_filename}_no_outliers_{timestamp}.png"

            print(f"\nProcessing {config_name} (excluding outliers from calculations)...")
...
def main():
    ...
    plotter = InterApproachPlotterNoOutliers(results_dir)
    plotter.generate_all_plots()

    print("\nInter-approach segmentation comparison plots (no outliers) completed!")
