#!/usr/bin/env python3
"""
Inter-Input Channel Comparison Box Plots for Cross-Validation Results (No Outliers)
...
IMPORTANT: This version EXCLUDES outliers from all calculations (median, quartiles, etc.)
using the IQR method (Q1 - 1.5*IQR to Q3 + 1.5*IQR).
"""
...
class InterInputPlotterNoOutliers:
    ...
    def remove_outliers_iqr(self, data):
        ...
        print(f"    Outlier removal: {len(data)} -> {len(filtered_data)} points (removed {len(data) - len(filtered_data)})")
    ...
    def load_hemisphere_data(self, file_path, approach):
        """Load hemisphere-specific data from Excel file and remove outliers"""
        try:
            if approach in ['single-class', 'single-class_halfdps', 'multi-label']:
                ...
                for hemi in ['Left', 'Right']:
                    ...
                    raw_data = hemi_data.tolist()
                    print(f"  {approach} {hemi} hemisphere:")
                    filtered_data = self.remove_outliers_iqr(raw_data)
                    data[hemi] = filtered_data
    ...
    def create_approach_plot(self, approach, files_dict, save_path):
        ...
        for config_name in ['CBF', 'CBF+T1w', 'CBF+FLAIR', 'CBF+T1w+FLAIR']:
            if config_name in files_dict:
                ...
                print(f"\nLoading {config_name} data (removing outliers):")
        ...
        # Since we've already removed outliers, we can show fliers (there shouldn't be any)
        box_parts = ax.boxplot(
            ...
            showfliers=True  # Can show fliers since outliers are already removed
        )
        ...
        # Customize the plot
        ax.set_title(f'{approach.replace("_", " ").title()} Segmentation: DSC by Input Configuration and Hemisphere (No Outliers)\n'
                     f'Cross-validation Results - Outliers Excluded from Calculations',
        ...
        print(f"Saved {approach} plot (no outliers): {save_path}")
    ...
    def generate_all_plots(self):
        ...
        print("Inter-Input Channel Comparison Box Plot Generator (No Outliers)")
        print("=" * 70)
        print("NOTE: Outliers are EXCLUDED from all calculations using IQR method")
        print("=" * 70)
        ...
        for approach, config_files in files_dict.items():
            ...
            save_path = self.plots_dir / f"inter_input_comparison_{approach}_no_outliers_{timestamp}.png"

            print(f"\nProcessing {approach} (excluding outliers from calculations)...")
    ...
    def create_summary_comparison(self):
        ...
        if summary_data:
            ...
            summary_path = self.plots_dir / f"inter_input_summary_stats_no_outliers_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
def main():
    ...
    plotter = InterInputPlotterNoOutliers(results_dir)
    plotter.generate_all_plots()

    print("\nInter-input channel comparison plots (no outliers) completed!")
