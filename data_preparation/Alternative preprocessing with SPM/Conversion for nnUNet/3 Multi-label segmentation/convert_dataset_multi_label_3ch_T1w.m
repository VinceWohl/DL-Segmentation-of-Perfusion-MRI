%% convert_dataset_multi_label_3ch_T1w.m
% Outputs (.nii, uncompressed):
%   imagesTr: _0000 = CBF_LICA, _0001 = CBF_RICA, _0002 = T1w_coreg from ssRICA
%   labelsTr: 4D NIfTI with two channels (uint8/uint16):
%             channel 1 = left mask (LICA), channel 2 = right mask (RICA)
%   imagesTs: channels only (no labels)
% Notes:
%   - Overlap is represented by [1,1] in the two label channels.
%   - Uses LICA CBF as reference; optional spacing enforcement.
%   - If LR flip is triggered by LICA mask COM, ALL arrays (CBFs, T1w, masks) are flipped.
...
% RICA-coreg T1w file name is subject-specific, formed as: 'anon_r' + sub + '_T1w.nii'
% e.g., sub='sub-p001' -> 'anon_rsub-p001_T1w.nii'
...
for i = 1:height(tbl)
    ...
    p_t1w_rica  = fullfile(base,'ssRICA','T1w_coreg', coreg_t1w_name(sub)); % <-- new third channel

    if ~(isfile(p_cbf_lica) && isfile(p_cbf_rica) && isfile(p_mask_lica) && isfile(p_mask_rica) && isfile(p_t1w_rica))
    ...
    info_ref = make_clean_ref_info_noFlip(info_ref_raw, FORCE_SPACING); % orthonormal header; keeps origin

    % RICA CBF
    [img_rica, ~] = read_nii_any(p_cbf_rica);
    if ~isequal(size(img_rica), ref_size)
        fprintf('[WARN] %s: RICA dims differ from LICA -> skipping\n', caseId); continue;
    end

    % RICA-coreg T1w (new)
    [img_t1w, ~] = read_nii_any(p_t1w_rica);
    if ~isequal(size(img_t1w), ref_size)
        fprintf('[WARN] %s: T1w_coreg dims differ from LICA -> skipping\n', caseId); continue;
    end
    ...
    if flipLR
        ...
        img_t1w  = flip(img_t1w,  1);   % <-- flip T1w as well
        Lmask    = flip(Lmask,    1);
        Rmask    = flip(Rmask,    1);
        if VERBOSE_LABELS
    ...
    switch split
        case 'training/validation'
            ...
            dst_img2 = sprintf('%s_%04d%s', dst_stem, 2, FILE_ENDING); % third channel path
            ...
            if OVERWRITE || ~(isfile(dst_img0) && isfile(dst_img1) && isfile(dst_img2) && isfile(dst_lbl))
                ...
                % Threshold to binary and stack as channels (left, right)
...
% Build 'anon_rsub-pXXX_T1w.nii' from 'sub-pXXX'
...
% ---------- Make header consistent with the data ----------
% ImageSize must equal size(img)
info.ImageSize = size(img);

% PixelDimensions must have one entry per dimension
...
nd = ndims(img);
if numel(pd) < nd
    pd = [pd, ones(1, nd - numel(pd))];   % e.g., add 1 for 4th dim (channels)
elseif numel(pd) > nd
    pd = pd(1:nd);
end
