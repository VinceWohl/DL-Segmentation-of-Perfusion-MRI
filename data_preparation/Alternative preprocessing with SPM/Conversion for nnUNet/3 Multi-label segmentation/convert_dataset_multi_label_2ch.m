%% convert_dataset_dual_channel_labels.m — same geometry & LR-fix; labels are 2-channel (L/R)
% Outputs (.nii, uncompressed):
%   imagesTr: _0000 = CBF_LICA, _0001 = CBF_RICA
...
% Notes:
%   - Overlap is allowed and represented by [1,1] at a voxel.
%   - Uses LICA CBF as reference for a clean, orthonormal header; optional spacing enforcement.
...
for i = 1:height(tbl)
    ...
    info_ref = make_clean_ref_info_noFlip(info_ref_raw, FORCE_SPACING); % orthonormal, keeps axis senses & origin
    ...
    % Masks (used for both Tr and Ts to decide flipping)
    ...
    switch split
        case 'training/validation'
            ...
            if OVERWRITE || ~(isfile(dst_img0) && isfile(dst_img1) && isfile(dst_lbl))
                % Write inputs
                write_nii_using_ref(img_lica, info_ref, dst_img0);   % _0000 = LICA
                write_nii_using_ref(img_rica, info_ref, dst_img1);   % _0001 = RICA

                % Threshold to binary and stack as channels
                L = uint8(double(Lmask) > LICA_THRESH);
                R = uint8(double(Rmask) > RICA_THRESH);
                Y2 = cat(4, L, R); % 4D volume, last dim = 2 channels
                ...
                % Write dual-channel label (uint8 by default; change to 'uint16' if desired)
        ...
        case 'testing'
            ...
            if OVERWRITE || ~(isfile(dst_img0) && isfile(dst_img1))
