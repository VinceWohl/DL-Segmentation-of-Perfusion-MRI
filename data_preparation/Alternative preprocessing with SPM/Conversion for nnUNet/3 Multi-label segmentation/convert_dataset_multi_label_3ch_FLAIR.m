%% convert_dataset_multi_label_3ch_FLAIR.m
% Outputs (.nii, uncompressed):
%   imagesTr: _0000 = CBF_LICA, _0001 = CBF_RICA, _0002 = FLAIR_coreg from ssRICA
...
%   - If LR flip is triggered by LICA mask COM, ALL arrays (CBFs, FLAIR, masks) are flipped.
...
% RICA-coreg FLAIR file name is subject-specific, formed as: 'anon_r' + sub + '_FLAIR.nii'
% e.g., sub='sub-p001' -> 'anon_rsub-p001_FLAIR.nii'
...
for i = 1:height(tbl)
    ...
    p_flair_rica= fullfile(base,'ssRICA','FLAIR_coreg', coreg_flair_name(sub)); % <-- third channel
    ...
    % RICA-coreg FLAIR (new)
    [img_flair, ~] = read_nii_any(p_flair_rica);
    if ~isequal(size(img_flair), ref_size)
        fprintf('[WARN] %s: FLAIR_coreg dims differ from LICA -> skipping\n', caseId); continue;
    end
    ...
    if flipLR
        ...
        img_flair = flip(img_flair,1);   % <-- flip FLAIR as well
        Lmask     = flip(Lmask,    1);
        Rmask     = flip(Rmask,    1);
        if VERBOSE_LABELS
    ...
    switch split
        case 'training/validation'
            ...
            if OVERWRITE || ~(isfile(dst_img0) && isfile(dst_img1) && isfile(dst_img2) && isfile(dst_lbl))
                ...
                write_nii_using_ref(img_flair, info_ref, dst_img2);   % _0002 = RICA FLAIR_coreg
...
% Build 'anon_rsub-pXXX_FLAIR.nii' from 'sub-pXXX'
