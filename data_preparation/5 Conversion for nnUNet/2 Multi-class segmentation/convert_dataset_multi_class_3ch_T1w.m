%% convert_dataset_multi_class_3ch_T1w.m
% nnU-Net formatting with 3 channels:
%   _0000 = CBF_LICA
%   _0001 = CBF_RICA
%   _0002 = T1w (coregistered from ssRICA: anon_rsub-pXXX_T1w.nii)
% labelsTr: uint16 with {0 bg, 1 left, 2 right, 3 overlap}
% Case-wise geometry:
%   - Use LICA CBF as reference
%   - Clean, orthonormal 3x3 (no flips), keep origin; optionally enforce spacing
%   - Data-driven L/R correction: if LICA mask COM lies in right half,
%     flip ALL arrays along x (dim 1), headers unchanged
% No JSONs here.
...
% ssRICA T1w coreg path builder uses: anon_r<sub>_T1w.nii  e.g. anon_rsub-p001_T1w.nii
...
for i = 1:height(tbl)
    ...
    % RICA CBF + RICA T1w (size checks)
    [img_rica, ~] = read_nii_any(p_cbf_rica);
    [img_t1w,  ~] = read_nii_any(p_t1w_rica);
    if ~isequal(size(img_rica), ref_size) || ~isequal(size(img_t1w), ref_size)
        fprintf('[WARN] %s: RICA/T1w dims differ from LICA -> skipping\n', caseId); continue;
    end
