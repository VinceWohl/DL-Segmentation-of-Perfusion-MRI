%% ========================================================================
% File: convert_dataset_multi_class_4ch.m
% Purpose:
%   Build nnU-Net-style dataset with FOUR input channels and a 4-class label.
%   Channels per case:
%     _0000 = CBF_LICA      (left perfusion)
%     _0001 = CBF_RICA      (right perfusion)
%     _0002 = T1w  (coreg)  (from ssRICA/T1w_coreg/anon_rsub-pXXX_T1w.nii)
%     _0003 = FLAIR(coreg)  (from ssRICA/FLAIR_coreg/anon_rsub-pXXX_FLAIR.nii)
%   Label: uint16 with classes {0=bg, 1=left, 2=right, 3=overlap}
%
% Geometry per case:
%   - Use LICA CBF as reference.
%   - Clean, orthonormal 3×3 (no axis flips), keep origin; optionally enforce spacing.
%   - L/R check: if LICA mask COM lies in the right half of the voxel grid,
%     flip ALL arrays (all 4 channels + both masks) along x (dim 1). Headers unchanged.
%
% Notes:
%   - Writes .nii (uncompressed) only. No dataset.json / splits here.
%   - Requires MATLAB built-ins: niftiread, niftiinfo, niftiwrite.
% ========================================================================
...
% ssRICA coreg files we will use for extra channels:
%   T1w   : .../ssRICA/T1w_coreg/anon_rsub-pXXX_T1w.nii
%   FLAIR : .../ssRICA/FLAIR_coreg/anon_rsub-pXXX_FLAIR.nii
...
for i = 1:height(tbl)
    ...
    p_t1w_rica   = fullfile(base,'ssRICA','T1w_coreg',   sprintf('anon_r%s_T1w.nii',   sub)); % 3rd channel
    p_flair_rica = fullfile(base,'ssRICA','FLAIR_coreg', sprintf('anon_r%s_FLAIR.nii', sub)); % 4th channel
    ...
    if ~isequal(size(img_rica), ref_size) || ~isequal(size(img_t1w), ref_size) || ~isequal(size(img_flair), ref_size)
        fprintf('[WARN] %s: a channel dims differ from LICA -> skipping\n', caseId); continue;
    end
    ...
    switch split
        case 'training/validation'
            ...
            if OVERWRITE || ~(isfile(dst_img0) && isfile(dst_img1) && isfile(dst_img2) && isfile(dst_img3) && isfile(dst_lbl))
                ...
                write_nii_using_ref(img_t1w,   info_ref, dst_img2);   % _0002 = RICA T1w
                write_nii_using_ref(img_flair, info_ref, dst_img3);   % _0003 = RICA FLAIR
