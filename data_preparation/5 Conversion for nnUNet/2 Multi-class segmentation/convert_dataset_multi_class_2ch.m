%% convert_dataset_multi_class_2ch.m — case-wise clean geometry + L/R fix by mask
% Outputs (.nii, uncompressed):
%   imagesTr: _0000 = CBF_LICA, _0001 = CBF_RICA
%   labelsTr: uint16 {0 bg, 1 left, 2 right, 3 overlap}
%   imagesTs: channels only
% Strategy:
%   1) Use LICA CBF as reference; build a clean, orthonormal 3x3 (no flips),
%      keep origin; optionally enforce spacing.
%   2) Read L/R masks; if the LICA mask’s COM lies on the RIGHT half of
%      the voxel grid, FLIP ALL ARRAYS (LICA/RICA/masks) along x (dim 1).
%      (Header stays the same; content moves to correct side.)
% No dataset.json / splits files here.
